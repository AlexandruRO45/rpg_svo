...
void IMUPreintegrator::Update(const Eigen::Vector3d &omega, const Eigen::Vector3d &acc,const double &delta_timestamp_){
  ...
  Eigen::Matrix3d dR = IMUSophus::Sophus::SO3::exp(omega*delta_timestamp_).matrix();
  Eigen::Matrix3d Jr_R = IMUSophus::Sophus::SO3::JacobianR(omega*delta_timestamp_);
  ...
  A.block<3,3>(3,6) = -delta_rot_ * IMUSophus::Sophus::SO3::hat(acc) * delta_timestamp_;
  A.block<3,3>(0,6) = -0.5* delta_rot_ *IMUSophus::Sophus::SO3::hat(acc) * dt2;
  ...
  jacobian_p_bias_gyroscope_ += jacobian_v_bias_gyroscope_ * delta_timestamp_ - 0.5 * delta_rot_ * IMUSophus::Sophus::SO3::hat(acc) * jacobian_rot_bias_gyroscope_ * dt2;

  jacobian_v_bias_accelerometer_ += -delta_rot_ * delta_timestamp_;
  jacobian_v_bias_gyroscope_ += -delta_rot_ * IMUSophus::Sophus::SO3::hat(acc) * jacobian_rot_bias_gyroscope_ * delta_timestamp_;
