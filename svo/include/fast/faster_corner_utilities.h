...
#define CHECK_BARRIER(lo, hi, other, flags)                                         \
    ...
    inline uint8x16_t load_ui128(const void *addr) { return vld1q_u8((const uint8_t *)addr); }
...
#define CHECK_BARRIER(lo, hi, other, flags)                                    \
    ...
    template <bool Aligned>
    inline __m128i load_si128(const void *addr) { return _mm_loadu_si128((const __m128i *)addr); }
    template <>
