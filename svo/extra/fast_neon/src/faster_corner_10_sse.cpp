...
void faster_corner_detect_10(const fast_byte* img, int img_width, int img_height, int img_stride,
  ...
  for (int y = 3; y < img_height - 3; y++) {
    ...
    for (int x = 16; x < xend; x += 16) {
      ...
      unsigned int ans_a, ans_c;
      {
        ...
        __m128i c =
            _mm_insert_epi16(_mm_srli_si128(a, 2), *(const unsigned short*)(p + 15 - stride), 7);
      ...
      unsigned int ans_d, ans_f;
      {
        ...
        __m128i f =
            _mm_insert_epi16(_mm_srli_si128(d, 2), *(const unsigned short*)(p + 15 + stride), 7);
